...
export default function Header({headerDetails, fontSize}){
    ...
    return (
        <Box>
            ...
            {headerDetails?.sold? <Text  fontWeight={600} fontSize={14} color={'#CBCBCB'} mt={6}>{headerDetails?.sold}</Text>:<Text mt={fontSize?6:''}  fontWeight={600}  fontSize={fontSize?fontSize:28} color={fontSize?'#FF6A6A':'black'} bgClip='text' bgGradient='linear(to-l, #CBCBCB, black)'> &#8358; {headerDetails?.amount}</Text>}
